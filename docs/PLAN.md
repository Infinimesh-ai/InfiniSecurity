...
**非 root 降级模式:递延,不列入任何里程碑(2026-07-30 决策)。**
本项目计划申请杀毒软件同类的平台认证——Linux 上 root 服务是常态;
未来若做 Windows / macOS 端拦截,走的同样是平台特权通道
(Windows:签名内核驱动 minifilter + WHQL;macOS:公证 + Endpoint
Security 系统扩展授权),那两个平台上也不存在"非特权降级"的位置。
真出现无法取得特权的场景再评估,届时如实标注其边界。
...
5. **默认 root 部署(5.0):** 防御系统与被监督用户之间必须隔一道特权
   ...
   二审 LLM 绝不跑 root;进程树内提权尝试默认 T3。
6. **非 root 降级模式递延:** 不列入里程碑。项目将申请杀软同类平台认证,
   Linux/Windows/macOS 的拦截全部走平台特权通道,降级模式没有位置;
   真出现拿不到特权的场景再评估。

## 8. 开放问题(动手对应里程碑前要定)

1. ~~保护目录集默认值与发现、策略防写、非 root 降级模式~~ 全部已决策:
   统一管理(2.4.2a)+ root 默认部署(5.0)+ 降级模式递延(第 7 节)。
   问题关闭。
