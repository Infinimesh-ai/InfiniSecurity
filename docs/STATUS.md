...
> 2026-08-07:隔离副本封存与保真回填已实现(单测 226 全绿,
> accept-m2.sh 新增 6 条断言),**待验收机重跑 M2 + 全量回归**。
> 详见下文"2026-08-07"节。
...
单元测试 226 个(纯逻辑,开发机可跑;2026-08-07 隔离副本封存一轮
+8);验收测试全部在虚拟机上以真实双用户布局执行,样本一律无害。
...
### 第七轮:把"改了但没验收覆盖"的清单补掉(154 → 210 PASS)

第六轮列出的覆盖缺口(`scripts/` 下对 io_uring / fallocate / openat2 / ioctl /
vanished / check-ignore **全部零命中**)已补。验收 154 → **210 PASS / 0 FAIL /
6 SKIP**,退出码全 0。

补上的关键覆盖:

- **拦截面完整性**(m1 ⑥,新增)。用 ctypes 发原始 syscall 观测,每条都做
  **归因对比**而不是只看成败:io_uring 三兄弟与 openat2 在监督下一律 ENOSYS,
  而基线下它们确实到达内核(EFAULT/ENOTSUP/EINVAL)——对比证明是过滤器干的,
  不是内核缺失。`fallocate` 纯预分配仍放行(白名单未误伤);PUNCH_HOLE /
  ZERO_RANGE 进判决;**未知位 WRITE_ZEROES(0x80)返回 EPERM 而不是 ENOTSUP**
  ——EPERM=被判决拒绝,ENOTSUP=溜过过滤器由内核拒绝,这个差别正是白名单
  方向的证据。`ioctl FS_IOC_ZERO_RANGE` 同理(EPERM vs 基线 EFAULT)。
- **判决缓存的 class + tier 键**(m2 ⑨,新增)。复审点名危险度最高的一条。
  9 次删除跑在**同一个 `infsec run`** 里——缓存是会话内的,一条命令一次
  `infsec run` 每次都是空缓存,键写错了也照样全绿。断言含:S3 秘密与 S4
  基础设施不被缓存放行、T1 授权不覆盖同前缀的 T2 操作,且每条都要求
  "走完整判决(rule 不以 cached-grant 开头)且等级 = 期望值";另有哨兵断言
  确认合并判决没被改坏(299 次缓存命中仍在)。
- **快照 vanished 判据**(m4 ⑦⑧,新增)。清单注入做确定性验证(目录消失
  一票否决、>5% 一票否决、少量仍通过、还原后重新通过的控制组),
  并发搬移做真实场景验证(窗口没命中时报 SKIP,绝不算 PASS)。
- **observe 三个分支**(m1 ④)。此前只覆盖 would-deny,现补 would-allow /
  would-review,并加"observe 没有代为执行"的归因断言。
- **`lsm status` 的两个健康信号**、LSM 重启后重新武装、隔离区的真实行为
  断言(m6,改用 asroot 建探针——命题本就是"连 root 也删不掉")。
- **三处靠陈旧证据通过的断言**(grep 整份持久化审计日志,历史上成功过一次
  就永远绿)改成只看本次新增行。
- m3 那个**从未执行过**的误报对照项(前置条件把"删除被拒"误判成"进程没起来")
  已修;thaw 之后闸门是否重新武装也补了覆盖。

**过程中的两次自我纠错值得记**:(1) 我给 m2 的 fixture 设计是错的——
`--may-delete` 对 S2 文件仍会被拒(S2 底线是 T2,预授权只在 class_floor ≤ T1
时才降级),唯一能让预授权真正改变判决的组合是 T2×S1;(2) 我新写的 fallocate
探针第一版把文件放在了保护集**之外**,于是断言变成在测内核而不是测过滤器,
第一次跑就红了——这恰好说明新断言必须先上机证明自己会变红。
...
- ~~**隔离副本仍可被原属主改写。**~~ **2026-08-07 已修**(见下节
  "隔离副本封存与保真回填"):每批次旁挂元数据清单 + 落盘后封存 +
  restore 精确回填。
- **隔离区/快照的中间目录仍是 0755,丢掉源目录原本的 mode。**
  文件**内容**的机密性已由封存(0440/0400,组读只给原属主主组)接住,
  但条目**名字**对本机其他用户可见;快照侧(`~/.ssh`、`~/.gnupg`
  在保护集里)整条未动。建议改成 0750 + 属组取被监督用户主组。

### 2026-08-07:隔离副本封存与保真回填(补掉头号已知缺口)

此前隔离副本仍是被监督用户属主、保留原 mode,可被事后清零——而
`quarantine list` 照样列出该条目,比直接删掉更迷惑人。第一次修复
(直接 `lchown root + chmod 0440`)因破坏 restore 保真度被撤回
(VM 实测恢复回来的文件 root:root 0440,用户读不了自己的数据)。
这次按当时记录的正确修法做完整:

1. **先记账**:每批次旁挂 `.infsec-meta.jsonl`(root 属主),逐条记录
   落点、原路径、原始 uid/gid/mode 与类型。清单写不进去就把原件移回
   原位并拒绝放行(fail-closed:没有账就没有保真恢复,宁可不删)。
2. **再封存**:副本 chown root、去全部写位;原属主的读位迁移到其
   **主组**(0440/0550)——"用户手工翻隔离区找回文件"这条路保持通畅
   (第三轮教训:把用户挡在自己备份外面是恢复工具最糟的失败方向),
   user-private-group 布局下组读恰好只等于原属主本人。封存经
   `O_PATH|O_NOFOLLOW` fd + `/proc/self/fd` 作用于 inode,不跟随符号
   链接(纪律 6);符号链接条目只 lchown 不 chmod;FIFO/socket 不封存。
3. **恢复时回填**:restore 在 rename **之前**拿住源条目的 O_PATH fd,
   成功后经同一个 fd 回填 uid/gid/mode——fd 指向 inode,与路径无关,
   不存在"回填到别的文件"的窗口。无清单的遗留批次照旧恢复、不动元数据。

开发中单测就抓到一处方向性缺陷:`rename(2)` 移动**目录**需要目录自身的
写权限(更新 `..` 项),封存态 0550 会挡住恢复(root 因 DAC override
不受影响,非 root 即失败)。已改为目录条目在改名前经 fd 解封、
改名失败再经同一个 fd 封回。

单测 218 → 226(封存 mode、0600/0755/0400 精确回填、遗留批次兼容、
清单不可见不可覆盖、空目录条目、跨文件系统副本、快照副本、符号链接
记账八项)。`accept-m2.sh` 新增 ①a 六条断言(副本属主 root、无写位、
原属主写不动、清单存在、清单不列出、restore 内容/mode/属主三头保真),
**待验收机重跑**——在 VM 全量回归通过之前,上表 M2 的数字不变、
本节不算验收完成。
